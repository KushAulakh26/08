...
const DEFAULT_CATS=[{id:'work',name:'Work',hex:'#555555'},{id:'personal',name:'Personal',hex:'#2d8a45'},{id:'meeting',name:'Meeting',hex:'#5a5a7a'},{id:'deadline',name:'Deadline',hex:'#ed4956'},{id:'event',name:'Event',hex:'#6a6a6a'}];
const CONTEXTS=[{id:'computer',label:'@Computer',icon:Monitor,color:'#a8a8a8'},{id:'calls',label:'@Calls',icon:Phone,color:'#a8a8a8'},{id:'home',label:'@Home',icon:Home,color:'#a8a8a8'},{id:'errands',label:'@Errands',icon:Archive,color:'#a8a8a8'},{id:'office',label:'@Office',icon:Briefcase,color:'#a8a8a8'}];
const ENERGY=[{id:'low',label:'Low',icon:Coffee,color:'text-neutral-500',bg:'bg-neutral-900',fill:'#6e6e6e'},{id:'medium',label:'Medium',icon:Zap,color:'text-neutral-400',bg:'bg-neutral-800',fill:'#a8a8a8'},{id:'high',label:'High',icon:Flame,color:'text-neutral-200',bg:'bg-neutral-700',fill:'#d4d4d4'}];
const TIME_EST=[5,15,30,45,60,90,120];
const REC_TYPES=[{v:'none',l:'None'},{v:'daily',l:'Daily'},{v:'weekly',l:'Weekly'},{v:'monthly',l:'Monthly'}];
const PROJ_COLORS=['#555555','#2d8a45','#5a5a7a','#ed4956','#6a6a6a','#4a7a7a','#7a4a5a','#5a7a40','#8a6a40','#4a7070'];
...
const hRgba=(hex,a)=>{if(!hex||hex.length<7)return`rgba(255,255,255,${a})`;const r=parseInt(hex.slice(1,3),16),g=parseInt(hex.slice(3,5),16),b=parseInt(hex.slice(5,7),16);return`rgba(${r},${g},${b},${a})`;};
...
:root{--bg:#000000;--surface:#121212;--surface2:#1a1a1a;--surface3:#262626;--border:rgba(255,255,255,0.08);--border2:rgba(255,255,255,0.15);--accent:#ffffff;--accent-dim:rgba(255,255,255,0.08);--accent-glow:rgba(255,255,255,0.06);--text:#ffffff;--text2:#a8a8a8;--text3:#6e6e6e;--emerald:#58c472;--amber:#e0a030;--rose:#ed4956;--violet:#a8a8a8;--radius:14px;--radius-sm:8px;--radius-lg:20px;}
...
.fm-input{width:100%;padding:9px 13px;background:var(--surface3);border:1px solid var(--border);border-radius:var(--radius-sm);color:var(--text);font-family:'DM Sans',sans-serif;font-size:13px;outline:none;transition:border-color .15s;}
.fm-input:focus{border-color:rgba(255,255,255,0.3);box-shadow:none;}.fm-input::placeholder{color:var(--text3);}
...
.fm-btn-primary{background:#ffffff;color:#000000;box-shadow:none;}
.fm-btn-primary:hover{background:#e0e0e0;box-shadow:none;}
.fm-btn-ghost{background:var(--surface3);color:var(--text2);border:1px solid var(--border);}
.fm-btn-ghost:hover{border-color:var(--border2);color:var(--text);}
.fm-btn-danger{background:rgba(237,73,86,.12);color:var(--rose);border:1px solid rgba(237,73,86,.15);}
.fm-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);transition:border-color .2s;}
.fm-card:hover{border-color:rgba(255,255,255,0.12);}
.nav-item{width:100%;display:flex;align-items:center;gap:9px;padding:8px 10px;border-radius:var(--radius-sm);margin-bottom:2px;cursor:pointer;border:1.5px solid transparent;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:500;text-align:left;background:transparent;transition:all .15s;color:var(--text2);-webkit-tap-highlight-color:transparent;}
.nav-item:hover{background:rgba(255,255,255,0.05);color:var(--text);}
.nav-item.active{background:rgba(255,255,255,0.08);color:#ffffff;border:1.5px solid rgba(255,255,255,0.1);}
.task-row{position:relative;border-radius:var(--radius);border:1px solid var(--border);cursor:pointer;overflow:hidden;transition:all .15s;background:var(--surface);-webkit-user-select:none;user-select:none;-webkit-tap-highlight-color:transparent;touch-action:manipulation;}
.task-row:hover{border-color:rgba(255,255,255,0.15);box-shadow:0 2px 8px rgba(0,0,0,.2);}
.task-row.selected{border-color:rgba(255,255,255,0.25);background:rgba(255,255,255,0.04);}
.task-row.done{opacity:.4;}
.task-row.overdue-glow{border-color:rgba(237,73,86,0.3);box-shadow:0 0 8px rgba(237,73,86,0.08);}
.task-row.priority-glow{border-color:rgba(224,160,48,0.2);}
...
.pill-tab{padding:5px 12px;border-radius:999px;font-size:11px;font-weight:600;cursor:pointer;border:1px solid var(--border);background:transparent;color:var(--text2);transition:all .15s;-webkit-tap-highlight-color:transparent;}
.pill-tab.active{background:#ffffff;color:#000000;border-color:#ffffff;}
.modal-overlay{position:fixed;inset:0;z-index:50;background:rgba(0,0,0,.75);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;}
.modal-box{background:var(--surface2);border:1px solid var(--border2);border-radius:var(--radius-lg);box-shadow:0 20px 60px rgba(0,0,0,.5);width:100%;max-width:480px;margin:0 16px;overflow:hidden;}
...
.empty-icon{width:64px;height:64px;border-radius:20px;background:var(--surface3);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;margin-bottom:16px;}
...
.progress-fill{height:100%;border-radius:4px;background:linear-gradient(90deg,#ffffff,#a8a8a8);transition:width .5s;}
...
.glow-dot{width:8px;height:8px;border-radius:50%;background:var(--emerald);box-shadow:0 0 0 3px rgba(88,196,114,.15);animation:pulse-glow 2s infinite;}
@keyframes pulse-glow{0%,100%{box-shadow:0 0 0 3px rgba(88,196,114,.15);}50%{box-shadow:0 0 0 6px rgba(88,196,114,.08);}}
.today-line-dot{position:absolute;left:-5px;top:-5px;width:10px;height:10px;border-radius:50%;background:var(--rose);box-shadow:0 0 6px rgba(237,73,86,.4);}
...
.cal-event{transition: filter .15s, transform .15s;-webkit-tap-highlight-color: transparent; -webkit-user-select: none;user-select: none;-webkit-touch-callout: none;touch-action: none;}.cal-event:hover{filter:brightness(1.2);}
...
.summary-card{border-radius:10px;padding:10px 14px;display:flex;align-items:center;gap:10px;border:1px solid var(--border);background:var(--surface);}
...
.quick-act:hover,.quick-act:active{background:rgba(255,255,255,0.06);color:var(--text);}
.reminder-card{background:var(--surface2);border:1px solid var(--border2);border-radius:12px;padding:10px 12px;width:280px;box-shadow:0 8px 24px rgba(0,0,0,0.4);}
.touch-ghost{position:fixed;z-index:100;pointer-events:none;padding:8px 16px;border-radius:10px;background:#ffffff;color:#000000;font-size:12px;font-weight:700;box-shadow:0 12px 40px rgba(0,0,0,0.5);max-width:200px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;display:flex;align-items:center;gap:6px;transform:translate(-50%,-130%);opacity:0.95;}
...
function MiniCal({cd,onSelect,events,sq,setSq}){
  ...
  return(<div className="select-none">
    ...
    {rows.map((row,ri)=><div key={ri} className="grid grid-cols-7">{row.map((cell,ci)=>{const sel=isS(cell),tod=isT(cell);return<button key={ci} onClick={()=>!cell.ov&&onSelect(vy,vm,cell.day)} className="relative flex items-center justify-center" style={{padding:'2px 0'}}><span style={{width:22,height:22,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:'50%',fontSize:10.5,fontWeight:tod?700:sel?600:400,background:tod?'#ffffff':sel?'rgba(255,255,255,0.1)':'transparent',color:tod?'#000000':sel?'#ffffff':cell.ov?'var(--text3)':'var(--text2)'}}>{cell.day}</span>{!cell.ov&&hasE(cell)&&!sel&&!tod&&<span style={{position:'absolute',bottom:0,left:'50%',transform:'translateX(-50%)',width:4,height:4,borderRadius:'50%',background:'var(--text2)'}}/>}</button>;})}</div>)}
  </div>);
}
...
function EvModal({isOpen,onClose,onSave,onDelete,event,selDate,ds,de,categories,onAddCat}){
  ...
  const[showAddCat,setShowAddCat]=useState(false);const[ncName,setNcName]=useState('');const[ncHex,setNcHex]=useState('#4a7a7a');
  const[alsoCreateTask,setAlsoCreateTask]=useState(false);
  const[startMin,setStartMin]=useState(540);const[endMin,setEndMin]=useState(600);
  const[startHour,setStartHour]=useState(9);const[startMinute,setStartMinute]=useState(0);
  const[endHour,setEndHour]=useState(10);const[endMinute,setEndMinute]=useState(0);

  useEffect(()=>{if(isOpen){setTitle(event?.title||'');setDesc(event?.description||'');setCat(event?.category||'work');setRec(event?.recurrence||'none');setRecEnd(event?.recurrenceEnd||'');setShowAddCat(false);setAlsoCreateTask(!event);const s=event?.startMin??ds??540;const e2=event?.endMin??de??600;setStartMin(s);setEndMin(e2);setStartHour(Math.floor(s/60));setStartMinute(s%60);setEndHour(Math.floor(e2/60));setEndMinute(e2%60);}},[event,isOpen,ds,de]);
  useEffect(()=>{const ns=startHour*60+startMinute;if(ns!==startMin){setStartMin(ns);if(endMin<=ns){const ne=Math.min(ns+30,1440);setEndMin(ne);setEndHour(Math.floor(ne/60));setEndMinute(ne%60);}}},[startHour,startMinute,startMin,endMin]);
  useEffect(()=>{const ne=endHour*60+endMinute;if(ne!==endMin){if(ne>startMin){setEndMin(ne);}else{const c=Math.min(startMin+30,1440);setEndMin(c);setEndHour(Math.floor(c/60));setEndMinute(c%60);}}},[endHour,endMinute,startMin,endMin]);

  if(!isOpen)return null;
  const cc=categories.find(x=>x.id===cat);const date=event?.date||selDate||fmtDO(new Date());
  const hourOptions=Array.from({length:24},(_,i)=>i);const minuteOptions=[0,15,30,45];
  ...
  return(<AnimatePresence>{isOpen&&<motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="modal-overlay" onClick={onClose}>
    <motion.div initial={{opacity:0,scale:0.92,y:24}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.92}} transition={spr} className="modal-box" onClick={e=>e.stopPropagation()}>
      <div style={{height:2,background:cc?.hex||'#555555'}}/>
      <div className="p-5" style={{maxHeight:'85vh',overflowY:'auto'}}>
        ...
        <div style={{background:'rgba(255,255,255,0.04)',border:'1px solid var(--border)',borderRadius:10,padding:'10px 12px',marginBottom:16,display:'flex',alignItems:'center',gap:10}}>
          <Calendar size={13} style={{color:'var(--text2)',flexShrink:0}}/>
          <div><p style={{fontSize:11,fontWeight:700,color:'var(--text)'}}>{new Date(date+'T00:00:00').toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric'})}</p></div>
        </div>
        <div style={{marginBottom:16}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
            <div style={{display:'flex',alignItems:'center',gap:4,background:'var(--surface3)',borderRadius:8,padding:'4px 6px',border:'1px solid var(--border)'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          ...
          <div><label style={{fontSize:10,fontWeight:700,color:'var(--text3)',textTransform:'uppercase',letterSpacing:'0.06em',display:'block',marginBottom:6}}>Category</label>
            <div style={{display:'flex',gap:6,flexWrap:'wrap',alignItems:'center'}}>
              {categories.map(c=><button key={c.id} onClick={()=>setCat(c.id)} style={{padding:'4px 12px',borderRadius:999,fontSize:11,fontWeight:700,cursor:'pointer',border:'none',background:cat===c.id?c.hex:hRgba(c.hex,0.2),color:'#fff'}}>{c.name}</button>)}
              <button onClick={()=>setShowAddCat(!showAddCat)} style={{width:24,height:24,borderRadius:'50%',border:'1px dashed var(--border2)',background:'transparent',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',color:'var(--text3)'}}><Plus size={10}/></button>
            </div>
            <AnimatePresence>{showAddCat&&<motion.div initial={{opacity:0,height:0}} animate={{opacity:1,height:'auto'}} exit={{opacity:0,height:0}} style={{overflow:'hidden'}}><div style={{marginTop:8,padding:10,background:'var(--surface3)',borderRadius:8,border:'1px solid var(--border)',display:'flex',flexDirection:'column',gap:8}}><input value={ncName} onChange={e=>setNcName(e.target.value)} placeholder="Category name..." className="fm-input" style={{fontSize:11,padding:'5px 8px'}}/><div style={{display:'flex',gap:5,flexWrap:'wrap'}}>{PROJ_COLORS.map(c=><button key={c} onClick={()=>setNcHex(c)} style={{width:18,height:18,borderRadius:'50%',background:c,border:'none',cursor:'pointer',boxShadow:ncHex===c?`0 0 0 2px var(--bg),0 0 0 3px ${c}`:'none'}}/>)}</div><button onClick={()=>{if(ncName.trim()&&onAddCat){onAddCat(ncName.trim(),ncHex);setNcName('');setShowAddCat(false);}}} className="fm-btn fm-btn-primary" style={{fontSize:11,padding:'5px 10px',alignSelf:'flex-start'}}><Plus size={10}/>Add</button></div></motion.div>}</AnimatePresence>
          </div>
          ...
          {!event&&<div>
            <button onClick={()=>setAlsoCreateTask(!alsoCreateTask)} style={{display:'flex',alignItems:'center',gap:8,padding:'10px 12px',borderRadius:10,border:'1px solid',cursor:'pointer',width:'100%',textAlign:'left',transition:'all 0.15s',background:alsoCreateTask?'rgba(88,196,114,0.06)':'transparent',borderColor:alsoCreateTask?'rgba(88,196,114,0.2)':'var(--border)'}}>
...
const TaskRow=memo(function TaskRow({task,showProject,showQuickActions,selected,projects,tags,scheduledMap,todayStr,onMark,onSelect,onMoveToTomorrow,onSnooze,onTouchStart}){
  ...
  const accent=isOverdue?'var(--rose)':task.priority?'var(--amber)':task.energy==='high'?'#d4d4d4':task.energy==='medium'?'#a8a8a8':'#3a3a3a';
  return(
      ...
      <div style={{padding:'9px 10px 9px 12px'}}>
        <div style={{display:'flex',alignItems:'flex-start',gap:8}}>
          <div style={{display:'flex',alignItems:'center',gap:4,marginTop:1,flexShrink:0}}>
            <GripVertical size={10} style={{color:'var(--text3)',cursor:'grab',opacity:0.3}}/>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:4,marginTop:5,flexWrap:'wrap'}}>
              {isOverdue&&<span className="badge" style={{background:'rgba(237,73,86,0.12)',color:'var(--rose)',fontSize:9}}><AlertCircle size={8}/>Overdue</span>}
              ...
              {schEv&&<span className="badge" style={{background:'rgba(255,255,255,0.06)',color:'var(--text2)',fontSize:9}}><Calendar size={8}/>{fmtS(schEv.startMin)}</span>}
          ...
          <div style={{display:'flex',alignItems:'center',gap:2,flexShrink:0,marginTop:1}}>
            ...
            <ChevronRight size={12} style={{color:selected?'var(--text)':'var(--text3)'}}/>
          </div>
        </div>
      </div>
    </motion.div>
...
export default function App(){
  ...
  const[categories,setCategories]=useState(DEFAULT_CATS);const[tags,setTags]=useState([]);
  ...
  const[delegateValue,setDelegateValue]=useState('');const[newSubInput,setNewSubInput]=useState('');
  ...
  const[newProjColor,setNewProjColor]=useState('#555555');const[newProjParent,setNewProjParent]=useState(null);
  const[confirmDeleteProj,setConfirmDeleteProj]=useState(null);
  const[showTagPicker,setShowTagPicker]=useState(false);const[newTagName,setNewTagName]=useState('');const[newTagHex,setNewTagHex]=useState('#7a4a5a');
  ...
  const[touchDragTask,setTouchDragTask]=useState(null);const[globalSearch,setGlobalSearch]=useState('');
  ...
  const isTouchRef=useRef(false);const touchColRef=useRef(null);const touchTaskRef=useRef(null);
  const longPressTimer=useRef(null);const taskLongPressTimer=useRef(null);const bodyScrollLockedRef=useRef(false);
  ...
  const renderCol=(dateStr,dayEvs,isToday,fullWidth,acceptDrop)=>{
    ...
    return(<div key={dateStr} ref={el=>{colRefs.current[dateStr]=el;}} style={{position:'relative',flex:1,minHeight:HH*24,borderRight:fullWidth?'none':'1px solid var(--border)',background:isToday?(dropHighlight?'rgba(255,255,255,0.04)':'rgba(255,255,255,0.02)'):''}} onMouseDown={e=>onColDown(e,dateStr)} onTouchStart={e=>onColTouchStart(e,dateStr)} onDragOver={acceptDrop?e=>{e.preventDefault();e.dataTransfer.dropEffect='copy';setDropHighlight(true);}:undefined} onDragLeave={acceptDrop?()=>setDropHighlight(false):undefined} onDrop={acceptDrop?e=>handleTaskDrop(e,dateStr):undefined}>
      ...
      {selection&&selection.dateStr===dateStr&&<div style={{position:'absolute',left:2,right:2,borderRadius:8,zIndex:10,pointerEvents:'none',top:mToY(selection.startMin),height:Math.max(mToY(selection.endMin-selection.startMin),16),border:'2px dashed rgba(255,255,255,0.3)',background:'rgba(255,255,255,0.05)'}}><div style={{padding:'2px 6px',fontSize:10,fontWeight:700,color:'var(--text)'}}>{fmtT(selection.startMin)} – {fmtT(selection.endMin)}</div></div>}
      ...
      {activeEvts.map(ev=>{const top=mToY(ev.startMin),height=Math.max(mToY(ev.endMin-ev.startMin),24);const c=getCat(ev.category);const evId=ev._bid||ev.id;
        return<div key={`a-${evId}`} data-event-id={evId} style={{position:'absolute',left:2,right:2,overflow:'hidden',color:'#fff',top,height,zIndex:50,borderRadius:10,cursor:movId?'grabbing':'ns-resize',transform:'scale(1.02)',boxShadow:`0 12px 32px rgba(0,0,0,0.5),0 0 0 2px ${hRgba(c.hex,0.5)}`}}>
          <div style={{position:'absolute',inset:0,background:`linear-gradient(135deg,${c.hex},${hRgba(c.hex,0.6)})`,borderRadius:10}}/><div data-resize="top" style={{position:'absolute',top:0,left:0,right:0,height:10,zIndex:30,cursor:'ns-resize'}}/>
          <div style={{position:'relative',padding:'4px 8px',pointerEvents:'none'}}><div style={{fontSize:11,fontWeight:700,overflow:'hidden',whiteSpace:'nowrap',textOverflow:'ellipsis'}}>{ev.title}</div>{height>=40&&<div style={{fontSize:10,opacity:0.8,marginTop:1}}>{fmtS(ev.startMin)} – {fmtS(ev.endMin)}</div>}</div><div data-resize="bottom" style={{position:'absolute',bottom:0,left:0,right:0,height:10,zIndex:30,cursor:'ns-resize'}}/>
        </div>;})}
    </div>);
  };
    ...
    return(<div style={{padding:16,overflowY:'auto',height:'100%'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:2}}>
          <button onClick={()=>{setFocusTaskId(selectedTask.id);setShowFocus(true);}} className="quick-act"><Play size={12} style={{color:'var(--text)'}}/></button>
      ...
      <div style={{display:'flex',flexWrap:'wrap',gap:4,marginBottom:12}}>
        ...
        {selectedTask.priority&&<span className="badge" style={{background:'rgba(224,160,48,0.12)',color:'var(--amber)'}}><Star size={9}/>Priority</span>}
        {selectedTask.dueDate&&<span className="badge" style={{background:selectedTask.dueDate<todayStr?'rgba(237,73,86,0.12)':'rgba(255,255,255,0.06)',color:selectedTask.dueDate<todayStr?'var(--rose)':'var(--text2)'}}><Calendar size={9}/>Due {selectedTask.dueDate}</span>}
        {selectedTask.startDate&&<span className="badge" style={{background:'rgba(88,196,114,0.1)',color:'var(--emerald)'}}><Calendar size={9}/>Start {selectedTask.startDate}</span>}
      </div>
      <div style={{display:'flex',flexDirection:'column',gap:12}}>
        ...
        <div><label style={{fontSize:10,fontWeight:700,color:'var(--text3)',textTransform:'uppercase',display:'block',marginBottom:6}}>Status</label>
          <div style={{display:'flex',flexWrap:'wrap',gap:5}}>{['next','waiting','someday','done'].map(s=><button key={s} onClick={()=>updateTask(selectedTask.id,{status:s,completedAt:s==='done'?todayStr:null})} style={{padding:'4px 10px',borderRadius:8,fontSize:11,fontWeight:700,cursor:'pointer',border:'1px solid',background:selectedTask.status===s?'#ffffff':'transparent',borderColor:selectedTask.status===s?'#ffffff':'var(--border)',color:selectedTask.status===s?'#000000':'var(--text2)',textTransform:'capitalize'}}>{s==='someday'?'Someday':s}</button>)}</div></div>
        <div><label style={{fontSize:10,fontWeight:700,color:'var(--text3)',textTransform:'uppercase',display:'block',marginBottom:6}}>Project</label>
          <div style={{display:'flex',flexWrap:'wrap',gap:5}}><button onClick={()=>updateTask(selectedTask.id,{project:null})} style={{padding:'4px 10px',borderRadius:8,fontSize:11,fontWeight:700,cursor:'pointer',border:'1px solid',background:selectedTask.project===null?'#ffffff':'transparent',borderColor:selectedTask.project===null?'#ffffff':'var(--border)',color:selectedTask.project===null?'#000':'var(--text2)'}}>None</button>{projects.map(p=><button key={p.id} onClick={()=>updateTask(selectedTask.id,{project:p.id})} style={{padding:'4px 10px',borderRadius:8,fontSize:11,fontWeight:700,cursor:'pointer',border:'1px solid',background:selectedTask.project===p.id?hRgba(p.color,0.2):'transparent',borderColor:selectedTask.project===p.id?p.color:'var(--border)',color:selectedTask.project===p.id?p.color:'var(--text2)'}}>{p.title}</button>)}</div></div>
        <div>
          ...
          <div style={{display:'flex',flexWrap:'wrap',gap:4,marginBottom:6}}>
            {tags.map(tg=>{const active=(selectedTask.tags||[]).includes(tg.id);return<button key={tg.id} onClick={()=>toggleTaskTag(selectedTask.id,tg.id)} style={{padding:'3px 9px',borderRadius:999,fontSize:10,fontWeight:700,cursor:'pointer',border:'1px solid',display:'flex',alignItems:'center',gap:3,background:active?hRgba(tg.hex,0.2):'transparent',borderColor:active?tg.hex:'var(--border)',color:active?tg.hex:'var(--text3)',transition:'all 0.15s'}}><Hash size={8}/>{tg.name}</button>;})}
            <button onClick={()=>setShowTagPicker(!showTagPicker)} style={{width:24,height:24,borderRadius:'50%',border:'1px dashed var(--border2)',background:'transparent',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',color:'var(--text3)'}}><Plus size={10}/></button>
          </div>
        ...
        <div>
          ...
          {stCount.t>0&&<div className="progress-bar" style={{marginBottom:8}}><div className="progress-fill" style={{width:`${stCount.t>0?(stCount.d/stCount.t)*100:0}%`,background:'linear-gradient(90deg,var(--emerald),#70d488)'}}/></div>}
          <div style={{background:'var(--surface3)',borderRadius:10,border:'1px solid var(--border)',padding:6,maxHeight:200,overflowY:'auto'}}>
      ...
      <div style={{display:'flex',gap:8,marginTop:14}}>
        <button onClick={()=>{setFocusTaskId(selectedTask.id);setShowFocus(true);}} className="fm-btn fm-btn-primary" style={{flex:1,justifyContent:'center'}}><Play size={11}/>Focus</button>
        <button onClick={()=>markDone(selectedTask.id)} className="fm-btn" style={{padding:'7px 12px',background:selectedTask.status==='done'?'rgba(88,196,114,0.1)':'var(--surface3)',color:selectedTask.status==='done'?'var(--emerald)':'var(--text2)',border:`1px solid ${selectedTask.status==='done'?'rgba(88,196,114,0.2)':'var(--border)'}`}}><CheckCircle size={11}/>{selectedTask.status==='done'?'Done':'Complete'}</button>
      </div>
    </div>);
  };
    ...
    return(<div style={{display:'flex',flex:1,overflow:'hidden'}}>
      ...
      <motion.div style={{height:'100%',display:'flex',flexDirection:'column',overflow:'hidden',background:'var(--bg)'}} animate={{flex:'1 1 auto'}} transition={sprG}>
        <div style={{flexShrink:0,padding:'12px 18px',borderBottom:'1px solid var(--border)'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:4}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:2,background:'var(--surface)',borderRadius:8,padding:'3px 4px',border:'1px solid var(--border)'}}>
          ...
          <div style={{display:'flex',gap:12,alignItems:'center'}}>
            <div style={{position:'relative',width:64,height:64,flexShrink:0}}>
              <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={ringData} innerRadius={22} outerRadius={30} startAngle={90} endAngle={-270} dataKey="v" stroke="none" isAnimationActive={true}><Cell fill="#ffffff"/><Cell fill="rgba(255,255,255,0.06)"/></Pie></PieChart></ResponsiveContainer>
        ...
        <div style={{flex:1,overflowY:'auto',padding:'12px 18px 16px'}}>
          ...
          <div><div style={{display:'flex',alignItems:'center',gap:6,marginBottom:10}}><Target size={14} style={{color:'var(--text)'}}/><h3 style={{fontSize:14,fontWeight:700,color:'var(--text)'}}>Task List</h3></div>
          ...
          {todayDoneCount>0&&<div style={{marginTop:20,padding:'10px 12px',background:'var(--surface)',borderRadius:12,border:'1px solid var(--border)'}}><div style={{display:'flex',alignItems:'center',gap:6,marginBottom:6}}><CheckCircle size={12} style={{color:'var(--emerald)'}}/><span style={{fontSize:11,fontWeight:700,color:'var(--text2)'}}>Completed Today ({todayDoneCount})</span></div><div style={{display:'flex',flexDirection:'column',gap:3}}>{tasks.filter(t=>t.status==='done'&&t.completedAt===todayStr).slice(0,5).map(t=><div key={t.id} style={{fontSize:12,color:'var(--text3)',textDecoration:'line-through',padding:'2px 0'}}>{t.title}</div>)}</div></div>}
        </div>
      </motion.div>
      <AnimatePresence mode="wait">{isDetOpen&&selectedTask&&(<motion.div key={selectedTask.id} initial={{opacity:0,x:50,flex:'0 0 0px'}} animate={{opacity:1,x:0,flex:'0 0 300px'}} exit={{opacity:0,x:50,flex:'0 0 0px'}} transition={sprG} style={{height:'100%',background:'var(--surface)',overflow:'hidden',borderLeft:'1px solid var(--border)'}}>{renderDetail()}</motion.div>)}</AnimatePresence>
    </div>);
  };

  const renderInboxView=()=>(<div><div className="section-header"><Inbox size={18} style={{color:'var(--text)'}}/><h2>Inbox</h2>{inboxTasks.length>0&&<span className="section-count">{inboxTasks.length}</span>}</div>{inboxTasks.length===0?<div className="empty-state"><div className="empty-icon"><CheckCircle size={24} style={{color:'var(--emerald)'}}/></div><p className="empty-title">Inbox Zero! 🎉</p></div>:<div style={{display:'flex',flexDirection:'column',gap:7}}><AnimatePresence>{inboxTasks.map(task=>(<motion.div key={task.id} layout initial={{opacity:0,y:6}} animate={{opacity:1,y:0}} exit={{opacity:0,x:-20}} transition={spr} className="fm-card" style={{padding:'10px 12px',display:'flex',alignItems:'center',gap:10}}><div style={{width:8,height:8,borderRadius:'50%',background:'var(--text2)',flexShrink:0}}/><div style={{flex:1,minWidth:0}}><p style={{fontSize:13,fontWeight:600,color:'var(--text)'}}>{task.title}</p><p style={{fontSize:10,color:'var(--text3)',marginTop:2}}>{task.createdAt}</p></div><button onClick={()=>{setProcTaskId(task.id);setProcStep(0);}} className="fm-btn fm-btn-primary" style={{padding:'5px 10px',fontSize:11}}><Brain size={10}/>Process</button><button onClick={()=>deleteTask(task.id)} className="quick-act"><Trash2 size={13}/></button></motion.div>))}</AnimatePresence></div>}</div>);

  const renderNextView=()=>{const filtered=ctxFilter==='all'?nextTasks:nextTasks.filter(t=>t.context===ctxFilter);return(<div><div className="section-header"><Zap size={18} style={{color:'var(--text)'}}/><h2>Next Actions</h2>{nextTasks.length>0&&<span className="section-count">{nextTasks.length}</span>}</div><div style={{display:'flex',gap:5,marginBottom:14,flexWrap:'wrap'}}><button onClick={()=>setCtxFilter('all')} className={`pill-tab ${ctxFilter==='all'?'active':''}`}>All</button>{CONTEXTS.map(ctx=>{const I=ctx.icon;return<button key={ctx.id} onClick={()=>setCtxFilter(ctx.id)} className={`pill-tab ${ctxFilter===ctx.id?'active':''}`} style={{display:'flex',alignItems:'center',gap:4}}><I size={10}/>{ctx.label}</button>;})}</div>{filtered.length===0?<div className="empty-state"><div className="empty-icon"><Zap size={24} style={{color:'var(--text2)'}}/></div><p className="empty-title">No actions</p></div>:<div style={{display:'flex',flexDirection:'column',gap:5}}><AnimatePresence>{filtered.map(t=><TaskRow key={t.id} task={t} selected={selectedTaskId===t.id} projects={projects} tags={tags} scheduledMap={scheduledMap} todayStr={todayStr} onMark={markDone} onSelect={selectTask} onMoveToTomorrow={moveToTomorrow} onSnooze={snoozeTask} onTouchStart={onTaskTouchStart}/>)}</AnimatePresence></div>}</div>);};

  const renderProjectsView=()=>(<div><div className="section-header" style={{justifyContent:'space-between'}}><div style={{display:'flex',alignItems:'center',gap:8}}><Folder size={18} style={{color:'var(--text)'}}/><h2>Projects</h2></div><button onClick={()=>setShowNewProj(!showNewProj)} className="fm-btn fm-btn-primary" style={{padding:'6px 12px',fontSize:11}}><Plus size={11}/>New</button></div>
    <AnimatePresence>{showNewProj&&(<motion.div initial={{opacity:0,height:0}} animate={{opacity:1,height:'auto'}} exit={{opacity:0,height:0}} style={{overflow:'hidden',marginBottom:16}}><div className="fm-card" style={{padding:16,display:'flex',flexDirection:'column',gap:10}}><input value={newProjTitle} onChange={e=>setNewProjTitle(e.target.value)} placeholder="Project name..." className="fm-input" autoFocus onKeyDown={e=>{if(e.key==='Enter')addProject();}}/><input value={newProjDesc} onChange={e=>setNewProjDesc(e.target.value)} placeholder="Description..." className="fm-input" style={{fontSize:12}}/><div><label style={{fontSize:10,fontWeight:700,color:'var(--text3)',textTransform:'uppercase',display:'block',marginBottom:7}}>Parent</label><div style={{display:'flex',gap:5,flexWrap:'wrap'}}><button onClick={()=>setNewProjParent(null)} style={{padding:'4px 10px',borderRadius:8,fontSize:11,fontWeight:600,cursor:'pointer',border:'1px solid',background:newProjParent===null?'#ffffff':'transparent',borderColor:newProjParent===null?'#ffffff':'var(--border)',color:newProjParent===null?'#000':'var(--text2)'}}>None</button>{projects.map(p=><button key={p.id} onClick={()=>setNewProjParent(p.id)} style={{padding:'4px 10px',borderRadius:8,fontSize:11,fontWeight:600,cursor:'pointer',border:'1px solid',background:newProjParent===p.id?hRgba(p.color,0.15):'transparent',borderColor:newProjParent===p.id?p.color:'var(--border)',color:newProjParent===p.id?p.color:'var(--text2)'}}>{p.title}</button>)}</div></div><div><label style={{fontSize:10,fontWeight:700,color:'var(--text3)',textTransform:'uppercase',display:'block',marginBottom:7}}>Color</label><div style={{display:'flex',gap:7,flexWrap:'wrap'}}>{PROJ_COLORS.map(c=><button key={c} onClick={()=>setNewProjColor(c)} style={{width:26,height:26,borderRadius:'50%',background:c,border:'none',cursor:'pointer',boxShadow:newProjColor===c?`0 0 0 2px var(--bg),0 0 0 4px ${c}`:'none',transform:newProjColor===c?'scale(1.25)':'scale(1)',transition:'all 0.2s'}}/>)}</div></div><div style={{display:'flex',gap:8}}><button onClick={addProject} className="fm-btn fm-btn-primary" style={{padding:'7px 16px'}}>Create</button><button onClick={()=>setShowNewProj(false)} className="fm-btn fm-btn-ghost">Cancel</button></div></div></motion.div>)}</AnimatePresence>
    {projects.length===0&&!showNewProj&&<div className="empty-state"><div className="empty-icon"><Folder size={24} style={{color:'var(--text2)'}}/></div><p className="empty-title">No projects yet</p></div>}
    <div style={{display:'flex',flexDirection:'column',gap:10}}>{rootProjects.map(p=><ProjectItem key={p.id} proj={p}/>)}</div>
  </div>);

  const renderWaitingView=()=>(<div><div className="section-header"><Clock size={18} style={{color:'var(--text)'}}/><h2>Waiting For</h2>{waitingTasks.length>0&&<span className="section-count">{waitingTasks.length}</span>}</div>{waitingTasks.length===0?<div className="empty-state"><div className="empty-icon"><Clock size={24} style={{color:'var(--text2)'}}/></div><p className="empty-title">Nothing pending</p></div>:<div style={{display:'flex',flexDirection:'column',gap:5}}>{waitingTasks.map(t=><TaskRow key={t.id} task={t} selected={selectedTaskId===t.id} projects={projects} tags={tags} scheduledMap={scheduledMap} todayStr={todayStr} onMark={markDone} onSelect={selectTask} onMoveToTomorrow={moveToTomorrow} onSnooze={snoozeTask} onTouchStart={onTaskTouchStart}/>)}</div>}</div>);
  const renderSomedayView=()=>(<div><div className="section-header"><Lightbulb size={18} style={{color:'var(--text)'}}/><h2>Someday / Maybe</h2></div>{somedayTasks.length===0?<div className="empty-state"><div className="empty-icon"><Lightbulb size={24} style={{color:'var(--text2)'}}/></div><p className="empty-title">No ideas yet</p></div>:<div className="fm-card" style={{overflow:'hidden'}}>{somedayTasks.map((task,i)=>(<div key={task.id} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',borderBottom:i<somedayTasks.length-1?'1px solid var(--border)':'none',cursor:'pointer'}} onClick={()=>selectTask(task.id)}><Lightbulb size={13} style={{color:'var(--text2)',flexShrink:0}}/><div style={{flex:1}}><p style={{fontSize:13,fontWeight:500,color:'var(--text)'}}>{task.title}</p></div><button onClick={e=>{e.stopPropagation();updateTask(task.id,{status:'next',context:suggestCtx(task.title),energy:suggestEn(task.title),timeEst:30});addToast('Activated!');}} className="fm-btn fm-btn-ghost" style={{padding:'4px 10px',fontSize:11}}><ArrowRight size={9}/>Activate</button></div>))}</div>}</div>);
  ...
  const renderReviewView=()=>{const doneChk=reviewChecks.filter(Boolean).length;const totalT=tasks.length;const doneC=doneTasks.length;const prodScore=totalT>0?Math.round((doneC/totalT)*100):0;const estMin=nextTasks.reduce((s,t)=>s+(t.timeEst||0),0);const estHrs=Math.round(estMin/60*10)/10;const stepCounts={inbox:inboxTasks.length,projects:projects.length,waiting:waitingTasks.length,someday:somedayTasks.length,plan:nextTasks.length};
    return(<div><div className="section-header"><RefreshCw size={18} style={{color:'var(--text)'}}/><h2>Weekly Review</h2></div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:16}}>{[{label:'Completion',value:`${prodScore}%`,sub:`${doneC}/${totalT}`,icon:TrendingUp},{label:'Inbox',value:inboxTasks.length,sub:inboxTasks.length===0?'Clear':'Process',icon:Inbox},{label:'Actions',value:nextTasks.length,sub:`~${estHrs}h`,icon:Zap},{label:'Review',value:`${doneChk}/5`,sub:doneChk===5?'Done!':'In progress',icon:RefreshCw}].map((card,i)=>{const CI=card.icon;return<div key={i} style={{borderRadius:14,padding:14,background:'var(--surface)',border:'1px solid var(--border)',color:'var(--text)',position:'relative',overflow:'hidden'}}><div style={{position:'absolute',right:-8,top:-8,opacity:0.06}}><CI size={56}/></div><p style={{fontSize:9,fontWeight:700,color:'var(--text3)',textTransform:'uppercase'}}>{card.label}</p><p style={{fontSize:22,fontWeight:800,marginTop:4,lineHeight:1}}>{card.value}</p><p style={{fontSize:10,color:'var(--text3)',marginTop:3}}>{card.sub}</p></div>;})}</div>
      <div className="fm-card" style={{overflow:'hidden'}}><div style={{padding:'8px 14px',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',gap:10}}><div className="progress-bar" style={{flex:1}}><div className="progress-fill" style={{width:`${(doneChk/5)*100}%`}}/></div><span style={{fontSize:11,fontWeight:700,color:'var(--text2)'}}>{doneChk}/5</span></div>
        {REVIEW_STEPS.map((step,i)=>{const SI=step.icon;const cnt=stepCounts[step.key];return(<div key={i} style={{display:'flex',alignItems:'flex-start',gap:10,padding:14,borderBottom:i<4?'1px solid var(--border)':'none',cursor:'pointer',background:reviewStep===i?'rgba(255,255,255,0.03)':'transparent'}} onClick={()=>setReviewStep(i)}><button onClick={e=>{e.stopPropagation();setReviewChecks(p=>{const n=[...p];n[i]=!n[i];return n;});}} style={{cursor:'pointer',border:'none',background:'transparent',padding:0,flexShrink:0}}>{reviewChecks[i]?<div style={{width:22,height:22,borderRadius:'50%',background:'var(--emerald)',display:'flex',alignItems:'center',justifyContent:'center'}}><Check size={11} style={{color:'#fff'}}/></div>:<div style={{width:22,height:22,borderRadius:'50%',border:'2px solid var(--text3)'}}/>}</button><div style={{flex:1}}><div style={{display:'flex',alignItems:'center',gap:6}}><SI size={13} style={{color:'var(--text2)'}}/><h3 style={{fontSize:13,fontWeight:700,color:reviewChecks[i]?'var(--text3)':'var(--text)',textDecoration:reviewChecks[i]?'line-through':'none'}}>{step.title}</h3>{cnt>0&&<span style={{fontSize:10,padding:'1px 7px',borderRadius:999,background:'var(--surface3)',color:'var(--text2)',border:'1px solid var(--border)',fontWeight:700}}>{cnt}</span>}</div><p style={{fontSize:11,color:'var(--text3)',marginTop:3}}>{step.desc}</p></div><button onClick={e=>{e.stopPropagation();setView(step.key==='plan'?'calendar':step.key==='projects'?'projects':step.key);}} className="quick-act"><ArrowRight size={10}/></button></div>);})}
      </div>
    </div>);
  };
    ...
    <div style={{display:'flex',height:'100vh',background:'var(--bg)',overflow:'hidden',fontFamily:'DM Sans,system-ui,sans-serif'}}>
      <AnimatePresence>{!sideCollapsed&&(
        <motion.aside initial={{width:0,opacity:0}} animate={{width:210,opacity:1}} exit={{width:0,opacity:0}} transition={sprG} style={{background:'var(--surface)',display:'flex',flexDirection:'column',flexShrink:0,zIndex:40,overflow:'hidden',borderRight:'1px solid var(--border)'}}>
          ...
          <div style={{padding:'8px 10px',borderTop:'1px solid var(--border)'}}><div style={{display:'flex',alignItems:'center',gap:8,padding:4}}><div style={{width:26,height:26,borderRadius:'50%',background:'var(--surface3)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,fontSize:11,fontWeight:700,color:'var(--text)'}}>U</div><div><p style={{fontSize:12,color:'var(--text)',fontWeight:600}}>User</p><p style={{fontSize:10,color:'var(--text3)'}}>Personal</p></div></div></div>
        </motion.aside>
      )}</AnimatePresence>
      <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        <div style={{height:44,flexShrink:0,borderBottom:'1px solid var(--border)',background:'var(--surface)',display:'flex',alignItems:'center',padding:'0 12px',gap:8}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:6}}>{(()=>{const v=NAV_VIEWS.find(n=>n.id===view);if(!v)return null;const Icon=v.icon;return<Icon size={13} style={{color:'var(--text2)'}}/>;})()}<span style={{fontSize:13,fontWeight:700,color:'var(--text)'}}>{NAV_VIEWS.find(n=>n.id===view)?.label||view}</span></div>
          <div style={{flex:1,maxWidth:280,position:'relative',marginLeft:16}}>
            ...
            {searchResults.length>0&&<div style={{position:'absolute',top:'calc(100% + 4px)',left:0,right:0,zIndex:60,background:'var(--surface2)',border:'1px solid var(--border2)',borderRadius:10,boxShadow:'0 8px 24px rgba(0,0,0,0.4)',maxHeight:320,overflowY:'auto'}}>
          ...
          <div style={{flex:1}}/>
          <button onClick={()=>setShowCapture(true)} className="fm-btn fm-btn-primary" style={{padding:'6px 12px',fontSize:12}}><Plus size={12}/>Capture <kbd style={{marginLeft:4,opacity:0.5,fontSize:9,background:'rgba(0,0,0,0.15)',borderRadius:3,padding:'1px 4px'}}>N</kbd></button>
        </div>
        ...
        <motion.div key="proc" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="modal-overlay" onClick={()=>{setProcTaskId(null);setProcStep(0);}}>
          <motion.div initial={{scale:0.92,opacity:0}} animate={{scale:1,opacity:1}} exit={{scale:0.92,opacity:0}} transition={spr} className="modal-box" onClick={e=>e.stopPropagation()}>
            <div style={{padding:'16px 18px'}}>
              <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12}}><Brain size={14} style={{color:'var(--text)'}}/><h3 style={{fontSize:15,fontWeight:800,color:'var(--text)'}}>Process Task</h3><button onClick={()=>{setProcTaskId(null);setProcStep(0);}} style={{marginLeft:'auto',padding:4,cursor:'pointer',border:'none',background:'transparent',color:'var(--text3)',borderRadius:6}}><X size={15}/></button></div>
              <div style={{background:'var(--surface3)',borderRadius:10,padding:'8px 12px',marginBottom:12,border:'1px solid var(--border)'}}><p style={{fontSize:13,fontWeight:700,color:'var(--text)'}}>{pt.title}</p></div>
              <AnimatePresence mode="wait">
                {procStep===0&&<motion.div key="s0" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} style={{display:'flex',flexDirection:'column',gap:6}}>
                  ...
                  {[{a:'next',icon:Zap,color:'#ffffff',bg:'rgba(255,255,255,0.06)',t:'Next Action',d:"I'll do this"},{a:'waiting',icon:User,color:'var(--text2)',bg:'rgba(255,255,255,0.04)',t:'Delegate',d:'Someone else'},{a:'someday',icon:Lightbulb,color:'var(--text2)',bg:'rgba(255,255,255,0.04)',t:'Someday',d:'Backlog it'},{a:'delete',icon:Trash2,color:'var(--rose)',bg:'rgba(237,73,86,0.08)',t:'Delete',d:'Not needed'}].map(o=>{const OI=o.icon;return<button key={o.a} onClick={()=>handleProcess(procTaskId,o.a)} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',borderRadius:10,border:'1px solid var(--border)',cursor:'pointer',textAlign:'left',background:'transparent',transition:'all 0.15s',fontFamily:'inherit',color:'inherit'}}><div style={{width:30,height:30,borderRadius:9,background:o.bg,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><OI size={13} style={{color:o.color}}/></div><div><p style={{fontSize:12,fontWeight:700,color:'var(--text)'}}>{o.t}</p><p style={{fontSize:11,color:'var(--text3)'}}>{o.d}</p></div><ArrowRight size={12} style={{color:'var(--text3)',marginLeft:'auto'}}/></button>;})}
                </motion.div>}
                {procStep===1&&<motion.div key="s1" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} style={{display:'flex',flexDirection:'column',gap:10}}>
                  <div><label style={{fontSize:10,fontWeight:700,color:'var(--text3)',textTransform:'uppercase',display:'block',marginBottom:6}}>Context</label><div style={{display:'flex',flexWrap:'wrap',gap:5}}>{CONTEXTS.map(ctx=>{const CI=ctx.icon;return<button key={ctx.id} onClick={()=>setProcCtx(ctx.id)} style={{padding:'4px 10px',borderRadius:8,fontSize:11,fontWeight:600,cursor:'pointer',border:'1px solid',display:'flex',alignItems:'center',gap:4,background:procCtx===ctx.id?'#ffffff':'transparent',borderColor:procCtx===ctx.id?'#ffffff':'var(--border)',color:procCtx===ctx.id?'#000':'var(--text2)'}}><CI size={10}/>{ctx.label}</button>;})}</div></div>
                  <div><label style={{fontSize:10,fontWeight:700,color:'var(--text3)',textTransform:'uppercase',display:'block',marginBottom:6}}>Energy</label><div style={{display:'flex',gap:5}}>{ENERGY.map(l=>{const LI=l.icon;return<button key={l.id} onClick={()=>setProcEnergy(l.id)} style={{padding:'5px 12px',borderRadius:8,fontSize:11,fontWeight:700,cursor:'pointer',border:'1px solid',display:'flex',alignItems:'center',gap:4,background:procEnergy===l.id?'#ffffff':'transparent',borderColor:procEnergy===l.id?'#ffffff':'var(--border)',color:procEnergy===l.id?'#000':'var(--text2)'}}><LI size={10}/>{l.label}</button>;})}</div></div>
                  <div><label style={{fontSize:10,fontWeight:700,color:'var(--text3)',textTransform:'uppercase',display:'block',marginBottom:6}}>Time</label><div style={{display:'flex',flexWrap:'wrap',gap:5}}>{TIME_EST.map(m=><button key={m} onClick={()=>setProcTime(m)} style={{padding:'4px 10px',borderRadius:8,fontSize:11,fontWeight:600,cursor:'pointer',border:'1px solid',background:procTime===m?'#ffffff':'transparent',borderColor:procTime===m?'#ffffff':'var(--border)',color:procTime===m?'#000':'var(--text2)'}}>{m}m</button>)}</div></div>
                  {projects.length>0&&<div><label style={{fontSize:10,fontWeight:700,color:'var(--text3)',textTransform:'uppercase',display:'block',marginBottom:6}}>Project</label><div style={{display:'flex',flexWrap:'wrap',gap:5}}><button onClick={()=>setProcProject(null)} style={{padding:'4px 10px',borderRadius:8,fontSize:11,fontWeight:600,cursor:'pointer',border:'1px solid',background:procProject===null?'#ffffff':'transparent',borderColor:procProject===null?'#ffffff':'var(--border)',color:procProject===null?'#000':'var(--text2)'}}>None</button>{projects.map(p=><button key={p.id} onClick={()=>setProcProject(p.id)} style={{padding:'4px 10px',borderRadius:8,fontSize:11,fontWeight:600,cursor:'pointer',border:'1px solid',background:procProject===p.id?hRgba(p.color,0.15):'transparent',borderColor:procProject===p.id?p.color:'var(--border)',color:procProject===p.id?p.color:'var(--text2)'}}>{p.title}</button>)}</div></div>}
                ...
                {procStep===3&&<motion.div key="s3" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} style={{display:'flex',flexDirection:'column',gap:10}}>
                  ...
                  <div style={{display:'flex',gap:8}}><button onClick={()=>setProcStep(0)} className="fm-btn fm-btn-ghost">← Back</button><button onClick={()=>handleProcess(procTaskId,'confirm-waiting')} className="fm-btn fm-btn-primary" style={{flex:1,justifyContent:'center'}}><Check size={12}/>Delegate</button></div>
                </motion.div>}
              </AnimatePresence>
            </div>
          </motion.div>
      ...
      <AnimatePresence>{showFocus&&(()=>{const ft=tasks.find(t=>t.id===focusTaskId);if(!ft)return null;return(
        <motion.div key="focus" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} style={{position:'fixed',inset:0,background:'var(--bg)',zIndex:50,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:32}}>
          <button onClick={()=>{setShowFocus(false);setPomRun(false);}} style={{position:'absolute',top:20,right:20,display:'flex',alignItems:'center',gap:6,padding:'6px 12px',borderRadius:8,cursor:'pointer',border:'1px solid var(--border)',background:'transparent',color:'var(--text2)',fontSize:11,fontWeight:600}}><Minimize2 size={12}/>Exit</button>
          <div style={{textAlign:'center',maxWidth:440}}>
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:10,marginBottom:14}}>
              <button onClick={()=>setPomRun(!pomRun)} style={{width:52,height:52,borderRadius:16,display:'flex',alignItems:'center',justifyContent:'center',border:'none',cursor:'pointer',background:pomRun?'var(--surface3)':'#ffffff',color:pomRun?'var(--text)':'#000000',boxShadow:'none'}}>{pomRun?<Pause size={20}/>:<Play size={20}/>}</button>
              <button onClick={()=>{setPomRun(false);setPomSec(pomMode==='work'?1500:300);}} style={{width:42,height:42,borderRadius:13,display:'flex',alignItems:'center',justifyContent:'center',border:'1px solid var(--border)',cursor:'pointer',background:'transparent',color:'var(--text2)'}}><RotateCcw size={15}/></button>
            </div>
            <div style={{display:'flex',justifyContent:'center',gap:7,marginBottom:32}}>{[{m:'work',l:'Focus 25m'},{m:'break',l:'Break 5m'}].map(o=><button key={o.m} onClick={()=>{setPomMode(o.m);setPomRun(false);setPomSec(o.m==='work'?1500:300);}} style={{fontSize:11,padding:'5px 14px',borderRadius:999,cursor:'pointer',border:'1px solid',background:pomMode===o.m?'rgba(255,255,255,0.08)':'transparent',borderColor:pomMode===o.m?'rgba(255,255,255,0.2)':'var(--border)',color:pomMode===o.m?'#ffffff':'var(--text3)',fontWeight:600}}>{o.l}</button>)}</div>
            <button onClick={()=>{markDone(ft.id);setShowFocus(false);setPomRun(false);}} className="fm-btn" style={{padding:'10px 24px',fontSize:13,background:'var(--emerald)',color:'#fff',fontWeight:700,border:'none'}}><CheckCircle size={14}/>Mark Complete</button>
          </div>
        </motion.div>);})()}</AnimatePresence>
